...
function renderLocations(locations) {
    $.each(locations, function(key, value) {
        $('#locations-table').append('<tr id="location-' + value.id + '">' +
            ...
            '            <td><a class="link-secondary" onclick="location.href=\'/locations/' + value.id + '\'">' + value.title + '</a></td>' +
            '            <td>' + value.locationType + '</td>' +
            '            <td>' +
            '                <button type="button" class="btn btn-danger" onclick="removeLocation(' + value.id + ')">Delete</button>' +
            '            </td>' +
            '            <td>' +
            '                <button type="button" class="btn btn-info" onclick="location.href=\'/locations/edit/' + value.id + '\'">Edit</button>' +
            '            </td>' +
            '        </tr>');
    });
}
